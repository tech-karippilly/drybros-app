...
import { CreateFranchiseRequest } from '@/lib/types/franchise';
...
export default function CreateFranchisePage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.code?.trim()) newErrors.code = 'Franchise code is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await franchiseService.createFranchise(cleanedData);
      ...
      if (error.response?.data?.message) {
        ...
        alert('Failed to create franchise. Please check your input and try again.');
      }
    } finally {
  ...
  return (
    ...
    >
      ...
      {/* Page Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white">Create New Franchise</h1>
        <p className="mt-1 text-sm text-gray-400">Add a new franchise location to your network.</p>
      </div>
