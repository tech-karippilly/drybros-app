...
import { ArrowLeft, Loader2, AlertCircle, MapPin, User, Calendar, Clock, DollarSign, CreditCard, FileText, CheckCircle, XCircle, UserPlus, Phone, CalendarClock, Ban, UserMinus, Search, Info, History, Star } from 'lucide-react';
import {
    getTripById,
...
export function TripDetailsScreen({ tripId, onBack }: TripDetailsScreenProps) {
    ...
    // For unassigned trips: load drivers via drivers API (by franchise), same design as Assign Driver screen
    useEffect(() => {
        if (!trip || trip.driverId || !trip.franchiseId) return;
        let cancelled = false;
    ...
    const scheduledAtUnassigned = trip?.scheduledAt;
    const scheduledTimeUnassigned = scheduledAtUnassigned ? formatTime(scheduledAtUnassigned) : '—';
    const scheduledDateUnassigned = scheduledAtUnassigned ? formatDate(scheduledAtUnassigned) : '—';
    const filteredDriversUnassigned = useMemo(() => {
                            ...
                            <div className="rounded-xl bg-slate-50 dark:bg-[#192633] overflow-hidden border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="p-4 space-y-4">
                                    <div className="relative pl-6 border-l-2 border-theme-blue/30 py-1">
                                        ...
                                        <div className="mb-4">
                                            <p className="text-[10px] text-slate-400 uppercase font-bold">{ASSIGN_DRIVER_STRINGS.PICKUP}</p>
                                            <p className="text-xs font-medium text-slate-900 dark:text-slate-200 truncate">
                                                {trip.pickupAddress || trip.pickupLocation || '—'}
                                            </p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400 uppercase font-bold">{ASSIGN_DRIVER_STRINGS.DESTINATION}</p>
                                            <p className="text-xs font-medium text-slate-900 dark:text-slate-200 truncate">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                                        <div>
                                            <p className="text-[10px] text-slate-400 uppercase font-bold">{ASSIGN_DRIVER_STRINGS.DATE}</p>
                                            <p className="text-xs font-semibold text-slate-900 dark:text-white">{scheduledDateUnassigned}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400 uppercase font-bold">{ASSIGN_DRIVER_STRINGS.TIME}</p>
                ...
                <section className="flex-1 flex flex-col overflow-y-auto">
                        ...
                        <div className="flex flex-wrap justify-between items-end gap-4 mb-8">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="bg-theme-blue/20 text-theme-blue px-3 py-1 rounded-full text-xs font-bold uppercase">
                                    {ASSIGN_DRIVER_STRINGS.SCHEDULED}: {scheduledTimeUnassigned}
                                </span>
                    ...
                    <div className="px-8 pb-10 space-y-4">
                        <h2 className="text-slate-900 dark:text-white text-lg font-bold px-1">
                            {ASSIGN_DRIVER_STRINGS.AVAILABLE_DRIVERS} ({filteredDriversUnassigned.length})
                        </h2>

                        {loadingDrivers ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                                <Loader2 className="size-10 animate-spin text-theme-blue" />
                                <p className="text-slate-500 dark:text-slate-400">{ASSIGN_DRIVER_STRINGS.LOADING_DRIVERS}</p>
                            </div>
                        ) : filteredDriversUnassigned.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4 bg-white dark:bg-[#192633] rounded-xl border border-slate-200 dark:border-slate-800">
                                ...
                                <p className="text-slate-600 dark:text-slate-300 font-medium">{ASSIGN_DRIVER_STRINGS.NO_DRIVERS}</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {filteredDriversUnassigned.map((driver) => (
                                    <div
                                        key={driver.id}
